...
export function formatSystemInstructionsWithMemories(baseInstruction: string, memories: Memory[]): string {
  ...
  let memoryBlock = 
    "\n\n" +
    "=== ELYSIA PERSISTENT MEMORY CORE (RECOLLECTIONS) ===\n" +
...
export async function processConversationSlice(
  ...
  try {
    ...
    const dialogueContext = dialogueHistory.map(line => `${line.role === "user" ? "User" : "Elysia"}: ${line.text}`).join("\n");

    const prompt = `You are Elysia's deep cognitive recollection engine. Your task is to analyze the recent conversation piece against previous persistent memories, and output precise update transactions.
...
- ACTIONS:
  ...
  - "REMOVE": If a memory was explicitly disproven or the user directly asked Elysia to forget it.
